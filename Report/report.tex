...
\newline
 The game became very popular in the 19th century, and the name Klondike is believed to have originated from the prospectors that were mining for gold in the Klondike region in Canada\cite{introduction}.
 \newline
...
The foundation piles are built up by stacking cards of the same suit starting from Ace and finishing with King. The goal of the game is to build up a stack of cards of the same suit. When this is accomplished, the game is won. 
\newline
...
\begin{figure}
	...
	\caption{Illustration of card layout in Solitaire made with Canva\cite{canva}}
...
The names of all transitions and places respect the restriction described in \cite{GPenSIMguide}, which states that only the 10 first characters of the names are significant. Acknowledging this, the first name of an element is always \textbf{t} or \textbf{p}, which means transition or place, respectively. \\

Next, the abbreviated name of the module will follow. The possibilities are \textbf{\ac{dp}, \ac{fp}, \ac{mc}, \ac{p}, \ac{pb},} and \textbf{\ac{tp}}. After this, either an underscore, \textbf{i} or \textbf{e} follows. The letter \textbf{i} means it's an internal transition, an \textbf{e} means it's and external transition. This is significant, as without these letters, the transitions \verb!tTPi_1_Move_Multiple! and \verb!tTPe_1_Move! would have a name collision. After either letter an underscore follows.\\

After the first underscore, the unique identifier will follow for the Tableau- and Foundation modules. The valid identifiers for \ac{tp} are \textbf{1, 2, 3, 4, 5, 6, 7}, and for \ac{fp} they are \textbf{Clubs, Diamonds, Hearts, Spades}. After the unique identifier another underscore will follow.\\
...
The model is developed using a Colored Petri Net, and relies heavily on colors to represent the different cards in the deck, and also to facilitate the communication between the different modules. The model is quite large, containing 94 transitions and 42 places. It is developed using the modular approach, and encompasses 5 different modules and a module connector. Some of the modules are duplicated, with the only difference being the names of the transitions and places.
\clearpage
...
\begin{table}
	...
	\begin{tabular}{|l|l|l|}
		...
		3  & pDP\_Draw\_FaceUp\_Pile   & Holds the face-up cards. Only the top card is visible to the player. \\ \hline
...
The second external input is \verb!tFPe_Clubs_Move!. It is used for moving cards to other modules, and works similarly to how movement is handled in the Draw Pile, with the additional caveat that all four Foundation Piles become enabled at the same time from \verb!pMC_FP_Move!. Due to this it's necessary to introduce additional logic to ensure that the issued move-command from the Player or Player Bot modules are meant for this particular module. As with the other modules, the actual validity of the move-command is handles by the \ac{p} and \ac{pb} modules.
...
The only external output of the {fp} is \verb!tFPe_Clubs_Out!. It works similarly to the output of the Draw Pile, where the pre-processor takes the latest arrived card from \verb!pFP_Clubs_Pile! and the earliest arrived command from \verb!pFP_Clubs_Move!. This ensures that the first issued command will be processed first, should there be more than one. The only time there would be more than one command executing concurrently is if neither the Player or the Player Bot modules where enabled, and the command was issued from another module which did not use the \verb!playerAction! resource. Listing \ref{lst:pre_tFPe_Out} shows parts of the code.
...
The Tableau Pile module is depicted in figure \ref{fig:tableau_pile}. It is duplicated 7 times, once for every pile in the tableau. The main differences between these modules are the names of their respective transitions and places, so the description given for the first pile will count for the other duplicates as well. Another difference is how many cards each pile is dealt during the initial phase. This is discussed in more detail in chapter \ref{sec:draw_pile} and \ref{sec1:solitaire_rules}. All the pre- and post-processor files are shared between all piles.
\begin{figure}
	\begin{center}
...
The only external output is \verb!tTPe_1_Out!, which will only fire once enough tokens are generated by \verb!tTPi_1_Move_Multiple!. In order to ensure that the correct card is moved, the pre-processor will fetch the \textbf{n}-th latest arrived card, where \textbf{n} is the number of tokens in \verb!pTP_1_Move!. This is repeated for every token in \verb!pTP_1_Move!. By doing this, correctness is preserved by always moving the highest ranking card first. Listing \ref{lst:pre_tTPe_Out} shows the main logic of the pre-processor, the whole file is found in at \ref{app:pre_tTPe_Out}.
...
\verb!global_info.BOT_ACTIONS! is a row vector with 4 elements numbered from 1 to 100. The bot has 5 possible commands; \verb!DP_Turn!, \verb!DP_Move!, \verb!FP_Move!, \verb!TP_Turn!, and \verb!TP_Move!. Given a random number generated from 1-100, the probability distribution for the commands is as follows; \verb!DP_Turn! between 1 and the first element of the vector, \verb!DP_Turn! between the first and second element, \verb!FP_Move! between the second and third element, \verb!TP_Turn! between the third and fourth element, and \verb!TP_Move! between the forth element and 100.
\verb!global_info.BOT_ACTIONS_TP_TP! defines how often (on average) the bot will attempt to move a card from a \ac{tp} pile to another \ac{tp} pile versus a \ac{fp} pile. Listing \ref{lst:main_simulation_file} shows this value set to 7, which means that the bot will opt to move to another \ac{tp} pile only 7 percent of the time.\\
...
By manipulation these values, the likelihood of solving the game will be drastically altered. Of course having a smarter bot which took actions after considering the game state, and perhaps learned from its mistakes would be much better. However, this was outside the scope of this project. Future work is discussed in more detail in chapter \ref{sec5_future_work}.
\\

Given that the bot continuously generates commands, even though they might be invalid for the current state of the game, I added the \ac{destroyer} \verb!tPBi_Destroyer!. This transition is not strictly necessary for the model to work, but was still deemed advantageous to keep the number of active tokens in the Petri Net as low as possible. Also it makes \verb!pPB_Cmd! \textbf{safe}. Listing \ref{lst:tPBi_Destroyer_pre} shows parts of the code for the \ac{destroyer}.
...
By having all the external inputs and output of every module be only transitions, we achieve a clean-cut interface. This also makes testing each individual module easier, as the desired colored input can be programmed in the pre-processor during testing. 
\\

The only pre- or post-processors of the module connector can be found in the \ac{destroyer} transitions. These are not strictly necessary in the current form of the model, as the Player and Player Bot modules do a good job of keeping tokens with incorrect colors out. However, it might be a good idea for future work to remove this responsibility from non-essential modules. Listing \ref{lst:tMC_FP_Move_Destroyer_pre} shows the destroyer of \ac{fp} move commands.
...
Each pile is indexed in order to make it easier to simulate moving cards between piles. Cards can be moved between piles by writing the amount of cards to be moved in the "To Pile" box - see Figure \ref{fig:gui_init}.
\newline
Should the "How many?" field be left blank, only one card will be moved.
Here the four suits are defined:
\begin{center}
	\begin{tabular}{ | l | l | l | }
		\hline
...
\begin{center}
	...
	\end{tabular}
\end{center}
I have also given index names to the seven Tableau Piles:
\begin{center}
	\begin{tabular}{ | l | l | l | }
		\hline
		Index name & Pile name \\ \hline
...
When an Ace is revealed in the initial position of the dealt cards or by shuffling through the Draw Pile, it can be moved to its corresponding foundation pile (FP\#).
...
In the event that a solution is found and the foundation piles are complete, the game ends and the final amount of points displayed as shown in figure \ref{fig:win}.
\begin{figure}
	\begin{center}
...
\lstinputlisting[style=matlabcode,caption=main\_simulation\_file.m lines 5-10,label=lst:main_simulation_file_3,firstline=5,lastline=10]{\GPenSIM/main_simulation_file.m}
\verb!global_info.DISP_CHANGES! will make the pre-processor display a simple statement every time they fire. This can be used for logging or further analysis.
\clearpage
\subsection{Commands}
\subsubsection{Move Command}~\\~\\
\label{sec:3_move_command}
The move command contains four parts; the command, destination, source and amount. Each part is concatenated, with colon as a separator. An example of a move command would be: \textit{Move:\ac{tp}1:\ac{tp}5:3}, which means \textit{Move 3 cards from \ac{tp}1 to \ac{tp}5}. If amount is not given, it will assume one card is to be moved.
\newline

In order to make sure of the validity of the move-commands, the function \verb!checkCommand_Move! has been developed. It is used both for validation before sending a command, and validation after receiving a command. The function takes the input parameters; \verb!command!, \verb!destination!, \verb!source!, and \verb!handle_err!. 
\newline
...
The only resource used in the model is called \verb!playerAction!. It serves an essential role of the model, which is to preserve the atomicity between the different actions. To explain why this is so important, one can imagine King of Spades is about to be moved to \ac{tp}4, which is empty. Only moments later, the command to move King of Hearts is also issued to be moved to \ac{tp}4. Now, since the second move command was issued before the first card was able to reach \ac{tp}4, it will pass the validity checks, and ultimately put the game in a completely illegal state.\\

The resource is acquired by requesting it every time either the Player or Player Bot attempts to issue a new command. If the resource is given, the name of the transition which owns it is stored in \verb!global_info.last_command_source!. Then the action can execute, knowing that no other action can execute concurrently. Once the action is complete, the resource is released in the post-processor. Listing \ref{lst:COMMON_POST_4} shows an example of the releasing of the resource.
...
\subsection{Scoring}
The standard scoring scheme in the Windows Solitaire game\cite{wiki} defines the amount of points that are awarded for moving cards between piles and is described as follows:
\begin{center}
	\begin{tabular}{ | l | l | l | }
		\hline
		Move & Points \\ \hline
...
When doing this, I had to remove arcs from the external inputs, as they would just be firing perpetually. Additionally, the names of the places and transitions were too long to be displayed in the graph, so I shortened them beforehand. A table which translates the names is included in each sub-chapter.
...
\begin{center}
	...
	\end{figure}
\end{center}
Figure \ref{fig:cov_tree_tableau_pile} shows the coverability tree generated for the Tableau Pile module. It's worth noting that the model as a whole is k-bounded even though the cycle indicator, \textbf{w} is shown in the coverability tree. The reason why \textbf{w} is shown, is that the \verb!cotree! function does not respect the pre-processor files of the transition.
\begin{center}
	\begin{tabular}{ | l | l | l | }
		\hline
		Name in Cov Tree & Actual name \\ \hline
...
\subsection{Results}
\label{sec:4_results}
There was not much time left to run a lot of simulations to figure out the best strategies. However, 100 games where run with the Player Bot enabled using the settings displayed in listing \ref{lst:main_simulation_file_2}. Out of these, 74 were solved successfully, whilst 26 entered an unwinnable state.
\lstinputlisting[style=matlabcode,caption=main\_simulation\_file.m lines 19-29,label=lst:main_simulation_file_2,firstline=19,lastline=29]{\GPenSIM/main_simulation_file.m}
\section{Discussion}
\subsection{Originality of this work}
I was not able to find any work that resembles this project. What differs in this project is that it contains a complete model of a game, and also a simple \ac{gui} to go along with it.
\subsection{Limitations of the project}
\label{sec5:limitations}
A major drawback of the destroyer \verb!tMC_Out_Buffer_Destroyer! is that if it fires, the card will actually be removed from the game, and the game becomes unsolvable. This transition will fire if the move-command of the token has an invalid destination. Due to how the Player and Player Bot modules are set up, this will never happen as they will check the validity of the move command before actually issuing the command. Still, I think it would be an improvement to add an additional transition to the Draw Pile module which would accept cards from \verb!tMC_Out_Buffer_Destroyer!, instead of totally discarding them.
\newline
...
A performance increase might be gained by reducing the number of transitions in the Player module, or at least introducing a new place and transition. This new place would serve as an input to all of the 20 transitions of the Player module, whilst the new transition would serve as an input the new place. By doing this, the number of enabled transitions, and pre-processors which would have to be executed would drastically drop.\\
...
\bibitem{introduction} Lee, Sloane \& Packard, Gabriel. 100 Best Solitaire Games: 100 Ways to Entertain Yourself with a Deck of Cards.; New York, N. Y.: Cardoza Publishing, 2004., \emph{ISBN: 1-58042-115-6}
\bibitem{Jupiter} Jupiter Scientific,\url{http://www.jupiterscientific.org/sciinfo/KlondikeSolitaireReport.html}
\bibitem{joelonsoftware} The Joel on Software Discussion \url{http://discuss.joelonsoftware.com/default.asp?joel.3.446403.10}
\bibitem{images} Images courtesy of freestock.com.
\bibitem{canva} Canva, \url{https://www.canva.com/}
\bibitem{wiki} Klondike (solitaire), \url{https://en.wikipedia.org/wiki/Klondike_(solitaire)}
